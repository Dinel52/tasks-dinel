...
import { useTranslation } from 'react-i18next'; // eslint-disable-next-line
...
function RegisterPage() {
  ...
  useEffect(() => {
    document.title = t('register.pageTitle', 'Register');
  }, [t]);
  ...
  return (
    <div className={`flex justify-center items-center min-h-screen ${darkMode ? 'bg-background' : ''} transition-colors duration-300`}>
      <RegisterForm darkMode={darkMode} toggleTheme={toggleTheme} />
          ...
          <DropdownMenuContent align="end" className={darkMode ? 'bg-gray-800 text-white border-gray-700' : ''}>
            ...
            >
              {i18nInstance.language === 'en'
                ? '✓ English'
                : `English (${t('language.english', 'English')})`
              }
            </DropdownMenuItem>
            <DropdownMenuItem
              ...
              className={darkMode ? 'hover:bg-gray-700 focus:bg-gray-700' : ''}
            >
              {i18nInstance.language === 'bs'
                ? '✓ Bosanski'
                : `Bosanski (${t('language.bosnian', 'Bosnian')})`
              }
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
